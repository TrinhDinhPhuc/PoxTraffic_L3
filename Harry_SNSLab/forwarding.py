...
def _install_path(prev_path, match):
	...
	if util.dpid_to_str(cur_sw) == "00-00-00-00-00-03" or util.dpid_to_str(dst_sw) == "00-00-00-00-00-03":
		log.debug(" Passed Successfully!")
	else:
		switches[dst_sw].connection.send(msg)  #this line may be :)	
	next_sw = cur_sw
	cur_sw = prev_path.prev[next_sw]
	while cur_sw is not None: #for switch in path.keys():
		msg = of.ofp_flow_mod()
		log.debug("actions: %s",(msg.actions))
		log.debug("buffer_id: %s",(msg.buffer_id))
		# log.debug("clone: %s",(msg.clone))
		log.debug("command: %s",(msg.command))
		log.debug("cookie: %s",(msg.cookie))
		log.debug("data: %s",(msg.data))
		log.debug("flags: %s",(msg.flags))
		log.debug("hard_timeout: %s",(msg.hard_timeout))
		log.debug("header_type: %s",(msg.header_type))
		log.debug("idle_timeout: %s",(msg.idle_timeout))
		# log.debug("match: %s",(msg.match))
		log.debug("out_port: %s",(msg.actions))
		# log.debug("pack: %s",(msg.pack))
		log.debug("priority: %s",(msg.priority))
		log.debug("xid: %s",(msg.xid))
		msg.match = match
		msg.idle_timeout = 10
		msg.flags = of.OFPFF_SEND_FLOW_REM
		log.debug("Current Switch: %s",util.dpid_to_str(cur_sw))
		log.debug("Next Switch:  %s", util.dpid_to_str(next_sw))
		log.debug("Next Port:  %s", adj[cur_sw][next_sw])
		msg.actions.append(of.ofp_action_output(port = adj[cur_sw][next_sw]))
		if util.dpid_to_str(cur_sw) == "00-00-00-00-00-03" or util.dpid_to_str(next_sw) == "00-00-00-00-00-03":
			log.debug(" Passed Successfully!")
		else:
...
class Switch(EventMixin):
	...
	def _handle_PacketIn(self, event):
		...
		if packet.effective_ethertype == packet.LLDP_TYPE:
			...
			#forward the packet ectly from the last switch, there is no need to have the packet run through the complete network.
			drop()
			dst = mac_learning[packet.dst]
			msg = of.ofp_packet_out()
			msg.data = event.ofp.data
